	...
	public postDBLoad(container: DependencyContainer): void {
		...
		if (config.SecureContainersOptions.enabled) {
			...
			if (config.SecureContainersOptions.Progressive_Containers.enabled) {
				try {
					...
					tables.hideout.production.push(...containerRecipes)
		...
		if (config.AdditionalCraftingRecipes.enabled == true) {
			try {
				...
				tables.hideout.production.push(...additionalCraftingRecipes)
			} catch (error) {
		...
		if (config.InsuranceChanges.enabled) {
			...
			try {
				...
				for (const loyaltyLevelID in prapor.base.loyaltyLevels) {
					prapor.base.loyaltyLevels[loyaltyLevelID].insurance_price_coef = 0.3
				}

				for (const loyaltyLevelID in therapist.base.loyaltyLevels) {
					therapist.base.loyaltyLevels[loyaltyLevelID].insurance_price_coef = 0.05
				}
			} catch (error) {
