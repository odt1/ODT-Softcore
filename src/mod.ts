...
class Mod implements IPostDBLoadMod {
	public postDBLoad(container: DependencyContainer): void {
		...
		if (config.OtherTweaks.enabled == true) {
					...
					if (item._type == "Item") {
						if (JSON.stringify(item).indexOf("ExcludedFilter") > -1) {
							...
							try {
								...
								logger.warning(
									`config.OtherTweaks.Remove_Backpack_Restrictions threw an error bacause of the other mod removed default item filter property (like Valens AIO). Ignore this error safely and continue.`
								)
								// log(error)
							}
						}
			...
			if (config.OtherTweaks.SICC_Case_Buff.enabled) {
				...
				try {
					...
					logger.warning(
						`config.OtherTweaks.SICC_Case_Buff threw an error because of the other mod (most likely Valen's AIO, because it removed default item property. Because of that, now SICC case allows all items.) Ignore and continue.`
					)
		...
		if (config.EconomyOptions.enabled) {
			if (config.EconomyOptions.BetterSalesToTraders.enabled) {
				if (debug) {
			...
			if (config.EconomyOptions.SkierUsesEuros.enabled) {
				// WIP
			...
			if (config.EconomyOptions.Disable_Flea_Market_Completely.disable == true) {
				...
				for (let handbookItem in tables.templates.handbook.Items) {
					...
					if (
						(!fleaListingsWhitelist.includes(itemInHandbook.ParentId) && config.EconomyOptions.Pacifist_FleaMarket.enabled == true) ||
