...
export class ScavCaseOptionsChanger {
	...
	private doBetterRewards() {
		...
		for (const item of potentialScavCaseRewards) {
			...
			if (item._parent === BaseClasses.AMMO_BOX) {
				...
				if (!count || !ammoType) {
					this.logger.warning("ScavCaseOptionsChanger: doBetterRewards: ammoType or count not found for itemTPl ${item._id}, skipping")
					continue
				}
	...
	private generatePotentialScavCaseRewards() {
		...
		return Object.values(items).filter((item) => {
			...
			if (item._type !== "Item" || this.scavCaseConfig.rewardItemBlacklist.includes(item._id) || this.itemFilterService.isItemBlacklisted(item._id)) {
				return false
			}
